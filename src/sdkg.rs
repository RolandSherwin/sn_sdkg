...
impl<N: NodeIdT> SyncKeyGen<N> {
    ...
    pub fn new<R: bls::rand::RngCore>(
        ...
        let our_idx = pub_keys
            .iter()
            .position(|(id, key)| *id == our_id && *key == sec_key.public_key())
            .map(|idx| idx as u64);
...
mod tests {
    ...
    #[test]
    fn non_participant_should_be_an_obeserver() -> Result<()> {
        let mut rng = bls::rand::rngs::OsRng;
        let threshold = 4;

        // Trusted set of nodes start a DKG round.
        let (mut nodes, parts) = init_nodes(7, threshold, &mut rng)?;
        // malicious node tries to participate in the DKG round by faking the Id; but it cannot obtain the part since
        // his PK is not part of the pre-determined set of `pub_keys`
        let mal_sk: SecretKey = bls::rand::random();
        let mal_id = 0;
        let pub_keys = nodes[&0].public_keys().clone();
        let (mal_key_gen, mal_part) =
            SyncKeyGen::new(mal_id, mal_sk.clone(), pub_keys, threshold, &mut rng)?;
        let mut mal_node = BTreeMap::from([(mal_id, mal_key_gen)]);
        assert!(mal_part.is_none());

        // The trusted nodes continue handling the parts
        let acks = handle_parts(&mut nodes, &parts, &mut rng)?;
        // malicious node cannot handle the parts
        assert!(handle_parts(&mut mal_node, &parts, &mut rng).is_err());

        // The trusted nodes continue handling the acks
        handle_acks(&mut nodes, &acks)?;
        // malicious node cannot handle the acks
        assert!(handle_acks(&mut mal_node, &acks).is_err());

        // The trusted nodes can now gen their shares
        let (sk_shares, pk_set) = gen_key_share(&mut nodes)?;
        // malicious node cannot generate the key share
        assert!(gen_key_share(&mut mal_node).is_err());

        verify_threshold(threshold, &sk_shares, &pk_set)?;

        Ok(())
    }

    #[test]
    fn nodes_should_require_all_acks_to_generate_key_share() -> Result<()> {
        let mut rng = bls::rand::rngs::OsRng;
        let threshold = 4;
        ...
        for (id, node) in nodes.iter() {
            let (_, sks) = node.generate()?;
            sk_shares.push((*id, sks.unwrap()));
        }
        // let pk_set = nodes[&0].generate()?.0;
        // attempt to subtract with overflow inside combine_signatures; cannot catch using `catch_unwind`
        // verify_threshold(threshold, &sk_shares, &pk_set);
